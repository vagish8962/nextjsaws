...
function FiltersSidebar(props) {
  ...
  const handleFilter = () => {
    const Checked = {
      BrandNames: [...checkedBrands],
      leapAttributes: [...checkedFilters]
    }
    ...
    <div className="basis-1/4 h-full sticky top-0">
      ...
      <article className="bg-white rounded-xl">
          ...
          <div className="space-y-2 mt-4">
            {brandData?.map((value) => (
              ...
              >
                ...
                <input
                  type="checkbox"
                  onChange={() => handleCheckBoxChangeBrand(value.brandName)}
                  className="form-checkbox mr-2 w-5 h-5 checbox-style"
                />
              </label>
            ))}
        ...
        <div className="w-full px-2 mb-4 mt-2 flex justify-center">
          <button onClick={handleFilter} className="bg-green-800 text-white rounded-xl py-3 px-6 font-unilevershilling w-64 mb-5">
            Apply Filters
          </button>
        </div>
      </article>
